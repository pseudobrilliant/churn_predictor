...
def test_processing(churn_scope):
    ...
    try:
        ...
        logging.error('test_analysis: Unable to create processed data set')
        raise
    ...
    for cat in cfg.categorical_columns:
        try:
            ...
            logging.error('test_analysis: processed data does not contain churn')
            raise
...
def test_train_models(churn_scope):
    ...
    for model in ['LogisticRegression', 'RandomForestClassifier']:
        try:
            file_name = f'{data_name}_{model}_model.pkl'
            assert os.path.exists(os.path.join(models_path,file_name))
        except AssertionError:
            logging.error('test_analysis: Unable to create saved model files')
            raise
        
        try:
            file_name = f'{data_name}_{model}_train_classifications.png'
            assert os.path.exists(os.path.join(model_rep_path,file_name))
            file_name = f'{data_name}_{model}_test_classifications.png'
            assert os.path.exists(os.path.join(model_rep_path,file_name))
        except AssertionError:
            logging.error('test_analysis: Unable to create classification reports')
            raise
        
    try:
        assert os.path.exists(os.path.join(model_rep_path,'all_models_roc.png'))
    except AssertionError:
        logging.error('test_analysis: Unable to create roc reports')
        raise
